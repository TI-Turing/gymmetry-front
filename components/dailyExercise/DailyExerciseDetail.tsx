...
export function DailyExerciseDetail() {
  ...
  const fetchOne = async () => {
    ...
    setError(null);
    try {
      const res = await dailyExerciseService.getDailyExerciseById(id);
      setItem(res.Data);
    } catch (_e) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      setError(null);
      try {
        const res = await dailyExerciseService.findDailyExercisesByFields({ Name: query.trim() } as any);
        let arr: any[] = [];
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>DailyExercise - Detalle</Text>
