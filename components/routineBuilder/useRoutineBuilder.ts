...
export function useRoutineBuilder() {
  ...
  const toggleDay = useCallback((dayNumber: number) => {
    setSelectedDays(prev => {
      if (prev.includes(dayNumber)) {
        ...
        setDays(d => [...d, { dayNumber, name: DAY_LABELS[dayNumber], exercises: [] }]);
        return [...prev, dayNumber].sort((a,b)=>a-b);
      }
    });
  }, []);

  const addExerciseToDay = useCallback((dayNumber: number, exercise: Exercise) => {
    setDays(prev => prev.map(d => d.dayNumber === dayNumber ? {
      ...d,
      exercises: [...d.exercises, { exercise, sets: 3, repsOrTime: '12', notes: '' }]
    } : d));
  }, []);

  const updateDayExercise = useCallback((dayNumber: number, index: number, patch: Partial<{sets: number; repsOrTime: string; notes: string;}>) => {
    setDays(prev => prev.map(d => d.dayNumber === dayNumber ? {
      ...d,
  ...
  const buildDraft = useCallback((): DraftRoutine => ({
